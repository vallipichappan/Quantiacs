...
def myTradingSystem(DATE, OPEN, HIGH, LOW, CLOSE, VOL, OI, P, R, RINFO, exposure, equity, settings):
    ...
    for market in range(nMarkets):
        # single exponential
        # fine tuned
        reg=SimpleExpSmoothing(CLOSE[:, market]).fit(smoothing_level=(1), optimized=False, use_brute=True)
        ...
        if trend[0] >= upperBand + (upperBand - lowerBand) * threshold:
            pos[market] = -1

        elif trend[0] <= lowerBand - (upperBand - lowerBand) * threshold:
            pos[market] = 1
