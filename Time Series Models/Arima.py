...
def myTradingSystem(DATE, OPEN, HIGH, LOW, CLOSE, VOL, OI, P, R, RINFO, exposure, equity, settings):
    ...
    for market in range(nMarkets):
        ...
        data = pd.Series(CLOSE[:, market]).apply(lambda x: float(x)).diff()
        data = data.drop(data.index[0]).reset_index().drop(columns = 'index')

        model = ARIMA(data, order=(0,0,0))
        model_fit = model.fit()
        trend = model_fit.forecast()

        

        if abs(trend[19])<threshold:
            trend[19] = 0
